...
- **Consequence:** Preflight disposition kanonikleşmiştir. Onaylı quantity-only slice (`4.0A` → `4.0B` → `4.0C` → `4.1`) tamamlanmıştır; quantity RECEIPT kernel, idempotent service, UI ve `inventory.receive_stock` permission rollout mevcuttur. Plain successful receipt ledger-only kalır; generic `AuditEvent` duplicate edilmez. ISSUE, TRANSFER, RETURN, correction, count/baseline ve serialized mutation bu karar kapsamı dışındadır. Açık hard gate'ler (`DEC-HG-001`, `DEC-HG-002`, `DEC-HG-005`) ve açık kararlar (`DEC-OPEN-004`, `DEC-OPEN-010`) status değiştirmeden korunur. Bu karar Gate 3 PASS anlamına gelmez. Quantity ISSUE first slice `DEC-027` ile ayrıca authorize edilmiştir.

### DEC-027 — Quantity ISSUE First-Slice Semantics and Sequencing

- **Status:** `DECIDED`
- **Preflight verdict:** `PASS FOR NEXT IMPLEMENTATION`
- **Required before:** Phase 4.2A quantity ISSUE kernel/schema extension implementation
- **Resolves:** Quantity ISSUE first-slice semantics, cardinality, condition policy, IssueContext contract, idempotency fingerprint, permission policy, audit boundary, DB integrity requirement and Phase 4.2A–4.2C sequencing
- **Does not resolve:** `DEC-HG-001` (count/baseline); `DEC-HG-002` (correction); `DEC-HG-005` (RETURN); `DEC-OPEN-004` (SerializedAsset); `DEC-OPEN-010` (UoM decimal-place enforcement); `DEC-OPEN-001` (condition availability effect); `DEC-OPEN-002` (multi-bucket/multi-location allocation); TRANSFER; RETURN; serialized inventory; correction; count/baseline; multi-line ISSUE; auto allocation/FIFO/FEFO
- **Decision:**
  1. **Second real inventory mutation:** quantity **ISSUE** only. Bu karar yalnız quantity ISSUE'yu authorize eder.
  2. **First ISSUE slice cardinality:** tek material, tek `source_location`, tek condition, tek quantity, tam olarak bir transaction line. Multi-line ISSUE yoktur. Lokasyon, condition veya balance'lar arasında otomatik allocation yoktur. FIFO / FEFO / auto-pick yoktur.
  3. **Condition policy:** Kullanıcı açıkça bir active `MaterialCondition` seçer. Herhangi bir active condition seçilebilir; `NEW_GOOD` gibi kodlar hard-code edilmez. ISSUE yalnız seçilen `(material, source_location, condition)` `StockBalance`'ından düşer. ISSUE condition dönüştürmez. `InventoryTransactionLine.condition`, kaynak bucket condition'ına eşittir. Condition'lar arası "available stock" aggregation tanıtılmaz. `DEC-OPEN-001` gelecek availability/eligibility policy için açık kalır.
  4. **ISSUE line shape:** `source_location` zorunlu; `target_location` null; `quantity > 0`; unit `Material.unit` ile eşleşmeli; `material.tracking_mode = QUANTITY`; condition açıkça seçilen active condition. Usage destination inventory target `Location` değildir; usage context `IssueContext`'tedir.
  5. **IssueContext (mandatory 1:1):** Her committed ISSUE için zorunlu. Receiver: `Employee` FK + `first_name`/`last_name`/`employee_number` snapshot. Production line: `ProductionLine` FK + `code`/`name` snapshot. Usage: zorunlu `usage_location_text` (trim leading/trailing whitespace; internal text/case korunur). Yeni ISSUE için receiver active `Employee`, `ProductionLine` active olmalıdır. Herhangi bir active ProductionLine node seçilebilir; leaf-only semantics yoktur. `UsagePlace` modeli yoktur. Snapshot'lar master edit sonrası değişmez; replay orijinal immutable historical data döner ve current mutable master state'i revalidate etmez.
  6. **Actor / receiver:** Actor = authenticated `User`; receiver = `Employee`. Receiver linked `User` gerektirmez. Actor, karşılık gelen `Employee` varsa kendine issue edebilir. Technician actor ve receiver olabilir; actor == receiver yasağı tanıtılmaz.
  7. **Concurrency / negative stock:** Negatif stok kesinlikle yasaktır. Yeni ISSUE: master lock/revalidate; exact `StockBalance` identity lock; missing balance => insufficient stock; locked quantity < requested => insufficient stock; ISSUE için missing `StockBalance` oluşturulmaz; decrement yalnız locked re-read sonrası; sıfır resulting balance korunur; ledger + IssueContext + balance decrement atomiktir. Concurrent last-stock ISSUE serialize edilir.
  8. **Idempotency:** `operation_id` + server-generated `request_fingerprint` (`DEC-009`). ISSUE fingerprint semantic identity: `transaction_type = ISSUE`, `acting_user_id`, `material_id`, `unit_id`, `condition_id`, `source_location_id`, quantity (3 decimal canonicalized), `receiver_employee_id`, `production_line_id`, canonical trimmed `usage_location_text`. Hariç: `operation_id`, `occurred_at`, master-derived snapshot strings, display labels. Aynı operation + aynı fingerprint => replay. Aynı operation + farklı fingerprint => conflict.
  9. **Permission / roles:** Managed permission: `inventory.issue_stock`. Runtime authorization yalnız Django permission; Group adı veya `is_staff` ile authorize edilmez. Fresh bootstrap şablonları: `TECHNICIAN`, `STOREKEEPER`, `ADMIN_MANAGER` + `issue_stock`. Mevcut Group'lar non-destructive kalır; otomatik reconcile yoktur. ISSUE UI için active ProductionLine seçimleri `inventory.issue_stock` ile yeterlidir; ISSUE dropdown için `view_productionline` TECHNICIAN/STOREKEEPER'a verilmez.
  10. **Audit boundary:** Başarılı ordinary ISSUE = `InventoryTransaction` + `InventoryTransactionLine` + `IssueContext`. Generic `AuditEvent`'e duplicate edilmez. Master-data/access mutation'ları mevcut `AuditEvent` davranışını korur.
  11. **Database integrity requirement:** IssueContext completeness/immutability yalnız application validation'a güvenemez. Phase 4.2A kernel implementation: her committed ISSUE tam bir IssueContext'e sahip olmalı; non-ISSUE transaction IssueContext sahiplenemez; persisted IssueContext UPDATE/DELETE edilemez; raw SQL bypass edilemez. Exact trigger/constraint implementation Phase 4.2A'ya aittir.
  12. **Implementation sequence (birleştirilmez):**
     - Phase 4.2A — ISSUE kernel/schema extension (transaction type ISSUE, type-specific line shape, nullable `target_location`, IssueContext, DB guards/immutability/completeness)
     - Phase 4.2B — quantity issue service (idempotency, locks, negative-stock prevention, projection-aware ISSUE, concurrency tests)
...
### DEC-037 — Inbound / Mal Kabul V1 Foundation

- **Status:** `DECIDED`
- **Required before / recorded with:** Inbound / Mal Kabul first production-safe slice
- **Extends:** `DEC-036` first-party identity; `DEC-003` source/target ledger semantics; `DEC-021` permission-based authorization; `DEC-023` Location capabilities
- **Does not resolve:** `DEC-OPEN-010` unit conversion; historical `material_code` global uniqueness / import matching remainder of `DEC-OPEN-021`; Talep Takip schema/UI; SKT/expiry workflow; supplier master; generic packaging conversion; inbound usage-place receipt metadata; warehouse-management task engine
- **Decision:**
  1. **Internal barcode is authoritative.** Operational identity `DEC-036` payload'larıdır: Material `TZ1M`, SerializedAsset `TZ1A`, Location `TZ1L`. UUID technical identity kalır. Tedarikçi/üretici barkodu yetkili envanter kimliği değildir ve TZ1 codec'ini değiştirmez.
  2. **Yeni Material insan kodu sistem üretir.** Format `MAT-` + 8 haneli ondalık (`MAT-00000001`). Kod opak ve kararlıdır; kategori, marka, lokasyon veya başka iş anlamı taşımaz. UUID ve TZ1M authoritative kalır. Kullanıcı yeni kayıtta kod uydurmak zorunda değildir. Mevcut/historical `material_code` değerleri geçerli kalır. Global uniqueness `DEC-OPEN-021` açık kalır; yalnız üretilen `MAT-[0-9]{8}` deseni kısmi unique constraint ile korunur. Backfill yoktur.
  3. **Material create permission.** Fresh `STOREKEEPER` ve `ADMIN_MANAGER` `catalog.add_material` alır. Fresh `STOREKEEPER` `catalog.change_material` almaz; mevcut malzeme düzenleme/pasifleştirme varsayılan STOREKEEPER yetkisi değildir. `ADMIN_MANAGER` mevcut management/change yetkisini korur. `TECHNICIAN` varsayılan olarak create almaz. Runtime authorization permission tabanlıdır; view/service Group adı kontrol etmez. `setup_roles` non-destructive kalır.
  4. **Model/MPN barkod yardımcısı.** Create/edit UX `Barkod Tara` sunar; USB HID/manuel ve mevcut yerel ZXing kamera altyapısı model alanını doldurur. Değer kayıttan önce düzeltilebilir. Okunan değer otomatik güvenilmez; tek başına Material veya stok oluşturmaz; form otomatik kaydedilmez. Opak sayısal/string değer kullanıcı-düzeltmeli metin olarak kabul edilir. External barcode registry bu dilimde yoktur.
  5. **Arama.** Liste/arama sistem kodu, ad, marka, model/MPN ve explicit `search_keywords` token'larını destekler. Fuzzy/AI/Elasticsearch/eş anlamlı motor yoktur. `technical_specs` JSON aranmaz. Anahtar yazımı korunur (yalnız virgül/boşluk ayrımı ve ilk yazıma göre dedup). Python `str.lower()` kullanılmaz; `İ` → `i` + U+0307 üretmez. Karşılaştırma/arama, `İ`→`i` ve `I`→`ı` sonrası Unicode lower uygulayan deterministik Turkish I katıdır. PostgreSQL `ILIKE`/`icontains` dotted/dotless I için collation'a bağlı kalır ve bu dilimde tek başına güvenilmez.
  6. **Miktar vs paket.** Malzemenin stok birimi authoritative kalır. Gelen fiziksel paket (ör. `2 KUTU × 1000 ADET`) stoğu sessizce çarpmaz. Generic UoM conversion engine yoktur; `DEC-OPEN-010` OPEN kalır. Paket metadata modeli bu dilimde yoktur.
  7. **Staging receive → putaway transfer.** Gelen malzeme fiziksel son rafa ulaşmadan kaydedilebilir. Kayıt gerçek stok tutan bir Location'a `RECEIVE` eder. Demo ortamı `MK` / `MK-BEKLEYEN` (`Mal Kabul` / `Yerleştirme Bekleyen`) seed eder; production kodu UUID/ad/kod sihirli sabitine bağlanmaz. Fiziksel yerleştirme mevcut `TRANSFER` servisiyladır. Sahte son-raf stoğu yoktur. WMS görev motoru yoktur.
  8. **Çoklu Location.** Aynı Material birden fazla stock-holding Location'da bulunabilir. STOREKEEPER gerçek hedefi seçer. Sistem tek sabit bin zorlamaz; otomatik putaway optimizer yoktur.
  9. **Location tarama.** Normal yerleştirme: gerçek Location etiketini oku/tara, hedefi doğrula, transfer et. Manuel seçim fallback'tir. Sunucu HID ile yazmayı ayırt ettiğini iddia etmez.
  10. **Etiket politikası.** Stock-holding raf/bin → `TZ1L`; QUANTITY Material kap/raf etiketi → `TZ1M`; SERIALIZED cihaz → `TZ1A`. Yeni Material veya ilk ilgili yerleşim sonrası `Etiket yazdır` teklif edilir; otomatik basım yoktur. Sıradan QUANTITY parça başına unique asset etiketi yoktur.
  11. **Kullanıldığı yer ≠ stok Location.** `SOĞUTMA KULESİ PANEL` / `TAVLAMA` kullanım/uygulama alanıdır; `Elektrik Ambarı → R03 → G07` fiziksel stok yeridir. Location bu bilgi için körlemesine yeniden kullanılmaz. Inbound usage/application metadata modeli bu dilimde yoktur; yanlış mevcut alana serbest metin yazılmaz.
  12. **SKT / expiry — onaylı gelecek iş.** SKT gelen parti/receipt bağlamına aittir, Material master'a değil. Yaklaşan/geçmiş SKT uyarısı ve personelin fiziksel ayırma/çıkarma işlemi V1 niyetidir. FEFO, otomatik stok düşümü, otomatik imha ve otomatik lot allocation yoktur. Bu dilimde uygulanmaz.
...
| `DEC-OPEN-021` | Material code uniqueness and format | `OPEN` | DM-B01, UF-O-17 | Catalog/import matching | İş sahibi + existing data | `DEC-037` ileriye dönük üretilen `MAT-[0-9]{8}` formatını ve yalnız bu desen için kısmi uniqueness'i kararlaştırır. Historical/non-generated `material_code` global uniqueness ve import matching açık kalır. Location code `DEC-023`; Employee number `DEC-024`. |
...
| Catalog/import identity matching | Serialized asset identity `DEC-032` ile kararlı; `DEC-037` yeni Material `MAT-########` üretimini kararlaştırır; historical uniqueness/import matching `DEC-OPEN-021` OPEN kalır (Location code `DEC-023`; Employee number `DEC-024`); history sonrası tracking mode için `DEC-013` zaten kararlı |
...
#### Phase 4.5 — Quantity TRANSFER First Slice

- **Disposition:** `COMPLETE` (`DEC-029`, 2026-09-12)
- **Kapsam:** TRANSFER kernel/schema/DB integrity guards; idempotent `transfer_quantity` service ve atomik source/target projection; create/detail UI; history/material integration; `inventory.transfer_stock` managed rollout.
- **Fresh role policy:** STOREKEEPER ve ADMIN_MANAGER alır; TECHNICIAN almaz. Managed permission count 23. `setup_roles` mevcut Group'ları non-destructive korur.
- **Audit boundary:** Successful ordinary TRANSFER yalnız immutable ledger'da izlenir; duplicate `AuditEvent`, `IssueContext` ve `TransferContext` yoktur.
- **Hard gate:** First slice COMPLETE; serialized TRANSFER, condition-changing TRANSFER, multi-source/multi-target, FIFO/FEFO, technician custody, person-to-person handover, production usage, correction/count ve QR/offline senaryoları deferred kalır.
