...
Phase 5.4D-A serialized physical-count backend COMPLETE'tir. Counting-owned `PhysicalCountSerializedLine` mevcut START adımında in-scope `SerializedAsset` expected snapshot'ını alır; bir oturum QUANTITY ve SERIALIZED sayım kanıtını birlikte taşıyabilir. Expected satırlar authoritative `SerializedAsset` snapshot referansıdır; mevcut authoritative asset beklenmeyen konumda gözlemlenebilir; fiziksel bulunan candidate item counting-owned staging olarak kaydedilir ve authoritative `SerializedAsset` oluşturmaz. Untouched expected asset explicit missing değildir; routine session `NOT_COUNTED` kabul eder, baseline-candidate completion serialized `NOT_COUNTED` reddeder. Counting non-authoritative kalır. `InventoryBaseline`, `INITIAL_BALANCE`, baseline establishment, serialized `COUNT_RECONCILIATION`, serialized discrepancy ledger etkisi, serialized ISSUE/RETURN/TRANSFER/controlled correction, custody, QR ve UI/permission rollout bu dilimde yoktur. Phase 5.3 authoritative `SerializedAsset` normalizasyon sınırı, baseline ikinci gerçek yazar olduğunda yeniden gözden geçirilmelidir; 5.4D-A candidate staging bu notu kapatmaz. Sonraki baseline/cutover implementation başlamamıştır.
