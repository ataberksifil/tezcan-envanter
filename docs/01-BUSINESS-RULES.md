...
| RCV-008 | DECIDED | Şirket Talep No takibi stok oluşturmaz. Karşılanan miktar yalnız gerçek `RECEIVE` bağlantısından türetilir (`DEC-039`). Tedarikçi serbest metindir; sipariş/fatura bu kuralın parçası değildir. | `RCV-005` satın alma master'ını açmaz. |
| RCV-009 | DECIDED | SKT gelen giriş kaydına aittir (`DEC-041`). Yaklaşan (30 gün, teknik varsayılan) ve geçmiş tarihler uyarılır. Fiziksel kontrol kaydı stok düşmez. FEFO, lot allocation ve otomatik imha yoktur. | Material master'a SKT yazılmaz. |
