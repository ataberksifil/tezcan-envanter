...
- Employee–ApplicationUser ilişkisi `DEC-024` ile kararlıdır: nullable one-to-one, ownership Employee; Phase 2.9B User.groups UI'sı Employee link yönetmez.
...
4. Talebi gönderir.
5. Talep `PENDING` olur. Yeni talep en az bir JPEG/PNG/WebP kanıt fotoğrafı taşır (`DEC-034`).

**Validation Rules**
- Talep eden ve trim edilmiş 10..2000 karakter açıklama zorunlu; correction target canonical original line olmalıdır (`DEC-030`).
- Yeni talep için en az bir geçerli kanıt fotoğrafı zorunludur; geçersiz dosya tüm oluşturmayı başarısız kılar (`DEC-034`).
- `PENDING` talep stok değiştirmez (COR-006).
- Sensitive fotoğraf public media URL ile sunulmaz; `corrections.view_correctionrequest` kontrol edilen `/corrections/evidence/<uuid>/` path üzerinden erişilir.

**Success Result**
- `correction_requests` ve bağlı `correction_evidence` kayıtları oluşturulur; özgün işlem değişmez.
...
**TBD / Open Decisions**
- Ret gerekçesi zorunluluğu yalnız `PROPOSED` (`DEC-OPEN-006`). Serialized correction deferred.
...
2. Yönetici talebi açar; özgün işlem/satır, açıklama, correction effect ve kanıt fotoğrafları incelenir.
...
- Evidence `DEC-034` ile V1 kapanmıştır; ret gerekçesi zorunluluğu yalnız `PROPOSED`dır. Karar anında yeni fotoğraf gerekmez; mevcut kanıt korunur.
...
**Phase 5.4 implementation note:** Physical Count + Combined Quantity/Serialized Baseline backend COMPLETE. Phase 5.4E default-role permission rollout tamamlanmıştır: fresh TECHNICIAN/STOREKEEPER `counting.view/add/change_physicalcountsession`; fresh ADMIN_MANAGER ayrıca `counting.decide_discrepancy` ve `imports.establish_baseline`. Count / baseline operational UI COMPLETE at `1dedd34051692e4c4743c63ead5fecd3c91e9229` (`feat: add count and baseline workflows`); `/counts/` ve `/baselines/`; migration yok; kernels/services authoritative kaldı. Serialized controlled correction, serialized `COUNT_RECONCILIATION` ve custody yoktur.
...
**Phase 5.4D-B/5.4E implementation note:** Backend ve default-role permission rollout COMPLETE. Operational baseline UI `/baselines/` altında COMPLETE at `1dedd34051692e4c4743c63ead5fecd3c91e9229` (`feat: add count and baseline workflows`). Sensitive `imports.establish_baseline` service-side uygulanır ve fresh ADMIN_MANAGER şablonunda kalır.
...
| Physical Count Sessions | `DEC-033` kararlı; Phase 5.4 backend + operational UI COMPLETE at `1dedd34` | UF-CNT-001 |
| Physical Count Entry | `DEC-033` kararlı; Phase 5.4 backend + operational UI COMPLETE at `1dedd34` | UF-CNT-001 |
| Reconciliation | `DEC-033` kararlı; QUANTITY kernel Phase 5.4C COMPLETE; quantity review/approve/reject UI COMPLETE at `1dedd34`; serialized `COUNT_RECONCILIATION` yoktur | UF-CNT-002 |
...
|---|---|
| `DEC-HG-001` | **DECIDED** (`DEC-033`); Phase 5.4A–5.4D-B counting/baseline backend + operational UI COMPLETE at `1dedd34051692e4c4743c63ead5fecd3c91e9229` (`feat: add count and baseline workflows`). |
