...
Transfer, count/reconciliation ve reporting permission'larını uydurma. `DEC-028` future direct quantity RETURN service'i için `inventory.return_stock` permission tanımını onaylar; managed role rollout ayrı görevdir. Rollout yapılana kadar `SAFE_CATALOG_PERMISSION_LABELS`, default role templates ve `setup_roles` grant'leri değiştirilmez.
