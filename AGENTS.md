...
Transfer, count/reconciliation ve reporting permission'larını uydurma. `DEC-028` direct quantity RETURN için `inventory.return_stock` permission'ını onaylar. Phase 4.4 rollout tamamlanmıştır: fresh `STOREKEEPER` ve `ADMIN_MANAGER` bu izni alır, `TECHNICIAN` almaz; managed permission count 22'dir. `setup_roles` non-destructive kalır ve mevcut Group'lara bu izni sessizce eklemez.
